...
本网关提供了一个命令行程序和一个调用库，命令行程序用来对外提供网关服务，调用库实现了和网关配套的客户端和服务端协议。
...
调用库实现了网关配套的通讯协议，网关通讯协议只实现到消息分包层面，应用层的消息序列化和反序列化网关并没有特殊要求。

可以使用任何形式的序列化格式，比如`Protobuf`、`JSON`等，只要能把应用层消息序列化成`[]byte`并反序列化回来就可以。

应用层的消息类型识别和派发也不在网关的职责范围内，消息类型识别可以参考[`link`](https://github.com/funny/link)项目在`codec`目录下的示例代码。

网关最后提供出来的通讯接口是基于`link`封装的，所以了解`link`的代码有助于理解网关调用库的使用。
